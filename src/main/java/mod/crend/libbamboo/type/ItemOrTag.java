...
public class ItemOrTag {
	 ...
	 *
	 * <p>When we open a config screen from the main menu, no world is loaded. Item tags are world-based, so we cannot
	 ...
	 *
	 * <p>Do note also that, without a world, these item tags are not actually loaded, and we only cache their names here.
	 ...
	 *
	 * <p>This method follows the same rules as {@link ItemOrTag#getItemTags()}.
	 *
	...
	public static boolean isItemTag(String value) {
		...
		try {
			TagKey<Item> tagKey = TagKey.of(RegistryKeys.ITEM, VersionUtils.getIdentifier(namespace, itemTag));
	...
	protected ItemOrTag(String identifier) {
		if (identifier.startsWith("#")) {
			...
			this.itemTag = TagKey.of(RegistryKeys.ITEM, VersionUtils.getIdentifier(tagName));
			this.isItem = false;
		} else {
	 ...
	 *
	 * <p>If this is an item variant, returns the item itself.
	 *
	 * <p>Else, returns the first item registered under the tag if it is loaded, or Air otherwise.
	 *
	 ...
	 *
	 * <p>If this is an item variant, checks for equality.
	 *
	 * <p>Otherwise, checks whether this item is part of this item tag.
